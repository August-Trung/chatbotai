...
class AppManager:
    def __init__(self, apps_json: str = "apps_index.json", alias_json: str = "alias_map.json"):
    ...
    def save(self):
        self.apps_path.write_text(json.dumps(self.apps, ensure_ascii=False, indent=2), encoding="utf-8")
        self.alias_path.write_text(json.dumps(self.alias, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def open_app(self, name_or_phrase: str) -> Tuple[bool, str]:
        ...
        if not canon:
            return False, f"Không tìm thấy ứng dụng phù hợp cho: {name_or_phrase}"
        cmd = self.apps.get(canon)
        if not cmd:
            return False, f"Ứng dụng '{canon}' chưa có cấu hình mở."
        try:
            if sys.platform.startswith("win"):
                subprocess.Popen(cmd, shell=True)
            elif sys.platform == "darwin":
                if canon in ("chrome", "facebook", "messenger", "tiktok", "youtube"):
                    subprocess.Popen(["open", "-a", "Google Chrome"] + cmd[2:] if len(cmd) > 2 else ["open", "-a", "Google Chrome"])
                else:
                    subprocess.Popen(["open", "-a", cmd[-1]])
            else:
                subprocess.Popen(cmd)
            return True, f"Đang mở {canon}."
        except Exception as e:
            log.error("open_app error for %s: %s", canon, e)
